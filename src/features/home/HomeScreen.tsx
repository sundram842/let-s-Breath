...
import { PRACTICE_OPTIONS, useBreathingSettings, type SessionConfig } from '@/features/settings';
...
export function HomeScreen() {
  ...
  const { practice, durations, session, hapticsEnabled, soundEnabled } = useBreathingSettings();
  const { canResume, beginNew } = useSession();

  const practiceOption =
    PRACTICE_OPTIONS.find((o) => o.value === practice) ?? PRACTICE_OPTIONS[0];
    ...
    >
      ...
      >
        <View style={styles.inner}>
          ...
          {/* Selected practice + rhythm */}
          <Animated.View
            entering={FadeInDown.duration(600).delay(80)}
            style={[styles.card, { backgroundColor: c.control }]}
          >
            <Text style={[styles.eyebrow, { color: c.subtitle }]}>SELECTED PRACTICE</Text>
            <Text style={[styles.practiceName, { color: c.title }]}>{practiceOption.label}</Text>
            <Text style={[styles.technique, { color: c.subtitle }]}>
              {practiceOption.technique}
            </Text>
