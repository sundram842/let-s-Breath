...
/** iOS Taptic impact styles we use (mapped to react-native-haptic-feedback). */
...
/** Our impact style → the react-native-haptic-feedback trigger name. */
const RNHF_TYPE: Record<
  Style,
  'impactLight' | 'impactMedium' | 'impactHeavy' | 'soft' | 'rigid'
> = {
  light: 'impactLight',
  medium: 'impactMedium',
  heavy: 'impactHeavy',
 ...
 * - iOS: the Taptic Engine via `react-native-haptic-feedback`; the intensity
 *   setting picks the impact styles, and sustained phases ramp them (inhale↑ / exhale↓).
 *
...
export class HapticEngine {
  ...
  private impact(style: Style): void {
    ReactNativeHapticFeedback.trigger(RNHF_TYPE[style], {
